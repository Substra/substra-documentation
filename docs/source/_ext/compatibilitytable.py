    ...
    def run(self):
        ...
        for release in releases["releases"]:
            ...
            for component_name in releases["components"]:
                ...
                app_para += nodes.reference(text=component["version"], refuri=component["link"], internal=False)
                if "helm" in component:
                    row += nodes.entry()
                    row[-1] += app_para
                    para =  nodes.emphasis()
                    para += nodes.reference(text=component["helm"]["version"], refuri=component["helm"]["link"], internal=False)
                    row += nodes.entry()
                    row[-1] += para
                else:
                    row += nodes.entry(morecols=1)
