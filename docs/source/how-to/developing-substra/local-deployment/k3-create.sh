...
mkdir -p /tmp/org-2
mkdir -p /tmp/org-3
k3d cluster create --api-port 127.0.0.1:6443 -p 80:80@loadbalancer -p 443:443@loadbalancer --registry-config /Users/gbarthes/Substra/substra-backend/registries.yaml --k3s-arg "--disable=traefik,metrics-server@server:*" --volume /tmp/org-1:/tmp/org-1 --volume /tmp/org-2:/tmp/org-2 --volume /tmp/org-3:/tmp/org-3
